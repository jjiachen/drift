...
for i in range(row_counts_users-1):
     City_users=sheet_users.col_values(6)[i+1]
     State_users=sheet_users.col_values(7)[i+1]
     geolocator = Nominatim()
     ...
     O="O%s"%(i+2)
     P="P%s"%(i+2)
     sheet_users.update_acell(O,lat)        #put the latitude in the googlesheets
     sheet_users.update_acell(P,lon)        #put the longitude in the googlesheets

for i in range(row_counts_deployers-1):
     City_EP_deployers=sheet_deployers.col_values(10)[i+1]
     State_EP_deployers=sheet_deployers.col_values(11)[i+1]
     geolocator = Nominatim()
     ...
     M="M%s"%(i+2)
     N="N%s"%(i+2)
     sheet_deployers.update_acell(M,lat)        #put the latitude in the googlesheets
     sheet_deployers.update_acell(N,lon)        #put the longitude in the googlesheets
"""
...
plot = GMapPlot(plot_width=1400, plot_height=800,x_range=DataRange1d(), y_range=DataRange1d(), map_options=map_options)
plot.title.text = "Drifter users (red), deployers (blue), and finders (green)"
...
color=[]
date_users=[]
for i in range(row_counts_users-1):      #add the different color
    color.append("red")
    date_users.append(" ") 
for i in range(row_counts_finders-1):
    color.append("green")
...
for i in range(row_counts_deployers-1):
    color.append("blue")
lon_degree=[float(i)+1 for i in lon_deployers if i not in ['None']]   #add 1 degree of longitude for each
lon_degree.insert(8,None)
lon_degree.insert(11,None)
lat_users.extend(lat_finders)
lat_users.extend(lat_deployers)
lon_users.extend(lon_finders)
lon_users.extend(lon_degree)
